...
@register_criterion('label_smoothed_cross_entropy_adapter_kd')
class LabelSmoothedCrossEntropyCriterion1(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, teacher_model=None, update_num=None):
        ...
        if self.num == 0:
            ...
            # 修改：教师模型采用自己：
            checkpoints_paths = ['/home/mc/adapter-kd/ckpt/teacher/deen/checkpoint_best.pt']
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, teacher_output=None, distil_strategy="normal",
        ...
        if distil_strategy == 'normal' or teacher_output is None:
        ...
        elif distil_strategy == 'adapter_kd':
            golden_loss, nll_loss = label_smoothed_nll_loss(
                lprobs, target, self.eps, ignore_index=self.padding_idx, reduce=reduce,
            )
            ...
            KL_loss1 = F.kl_div(lprobs, distil_lprobs, reduction='none')
            KL_loss1 = KL_loss1.sum(dim=-1)
            KL_loss1.masked_fill_(pad_mask, 0.)
            KL_loss1 = KL_loss1.sum()
            extra_result['KD_loss'] = KL_loss1

            KL_loss2 = F.kl_div(lprobs, nmt_prob, reduction='none')
            KL_loss2 = KL_loss2.sum(dim=-1)
            KL_loss2.masked_fill_(pad_mask, 0.)
            KL_loss2 = KL_loss2.sum()

            loss = golden_loss +  KL_loss1 + KL_loss2
