...
@register_criterion('label_smoothed_cross_entropy-step1')
class LabelSmoothedCrossEntropyCriterion1(FairseqCriterion):
    ...
    def __init__(self, task, sentence_avg, label_smoothing):
        ...
        self.combiner = Combiner(lambda_=0.9999,
                                 temperature=100, probability_dim=10152)
        self.num = 0
        self.teacher_model = None

        self.distil_strategy = 'distil_direct'
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, teacher_output=None, distil_strategy="normal",
        ...
        if distil_strategy == 'normal' or teacher_output is None:
            ...
            # 检查目标概率是否大于0.5
            # target0 = target.squeeze()
            # rows = torch.arange(distil_lprobs.size(0), device=distil_lprobs.device)
            ...
            mask = (knn_target_values >= 0.5).float()

            weighted_knn_prob = 0.5 * kNN_prob * mask
            weighted_nmt_prob = nmt_prob * (1 - 0.5 * mask)
