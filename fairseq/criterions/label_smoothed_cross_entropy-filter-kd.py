...
@register_criterion('label_smoothed_cross_entropy-filter-kd')
class LabelSmoothedCrossEntropyCriterion1(FairseqCriterion):
    ...
    def __init__(self, task, sentence_avg, label_smoothing):
        ...
        self.prior_tau = 1.0
        self.datastore = Datastore.load("/path/your_datastore_path", load_list=["vals"])
        self.datastore.load_faiss_index("keys")
        self.retriever = Retriever(datastore=self.datastore, k=16)
        self.combiner = Combiner(lambda_=0.999,
                                 temperature=10, probability_dim=42024)
        self.num = 0
        self.teacher_model = None

        self.distil_strategy = 'distil_filter'
    ...
    def get_teacher_probs(self, teacher_output):
        ...
        #获取校准后的kNN分布
        knn_prob = self.combiner.get_scaled_knn_prob(**self.retriever.results, device='cuda:0')
        # knn_prob = self.combiner.get_scaled_knn_prob(**self.retriever.results, device='cuda:0',
        #                                              cache_hidden=self.cache_hidden,
        #                                              current_gen_len=self.current_gen_len,
        #                                              source_weight=self.args.source_weight)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, teacher_output=None, distil_strategy="normal",
        ...
        elif distil_strategy == 'distil_filter':
